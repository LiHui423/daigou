				...
				var itemArr = [
					'<div class="swiper-slide">',
						'<div class="swiper-zoom-container">',
							'<img src="'+y+'">',
							'<span class="PicShopSize">'+ChooseArray[k].split('|').join('')+'</span>',
						'</div>',
					'</div>'
				];
			}else{
	...
	function neww(){
		...
		var up  = window.userPic;
		var code = -1;

		if(up==''||undefined){
			
			$('.whoUser').css('background','url('+window.osURL+'/images/nohead.png'+window.ys+') no-repeat center');
		}
		else{
			$('.whoUser').css('background','url('+window.osURL+up+window.ys+') no-repeat center');
			
		}
		if(un.length<4){
			var realLength = 0, len = un, charCode = -1;
				for (var i = 0; i < 2; i++) {
					charCode = len.charCodeAt(i);
					if (charCode >= 0 && charCode <= 128){
						realLength += 1;
			...
			un = un.substring(0,2)+'***'+un.substr(-1);
		}
