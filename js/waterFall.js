...
function waterfall(outer, item, num, margin, width, flag) {
	...
	num = num == -1 ? Math.floor(outerWidth/itemWidth) : num;
	var totalWidth = outer.width();
	flag = flag == undefined ? false : flag;
	width = flag ? (totalWidth - num * itemWidth) / (num - 1) : width;
	var eachWidth = itemWidth + width;
	var columNum = num;
	var margin = margin;
	var heightArr = [];
...
function easyScrollRequest(openState, evenSelector, viewport, context, callback){
